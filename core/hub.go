...
import (
        ...
        "runtime/debug"
        "strconv"
        "sync" // IMPL-007: needed for request buffer mutex
        "time"
)
...
// IMPL-007: Request batching buffer to reduce CGO crossings
var (
        requestBufMu   sync.Mutex
        requestBuf     []statistic.Tracker
        requestFlusher *time.Ticker
        requestDone    chan struct{}
)

// IMPL-007: Start the request batch flush goroutine
func startRequestBatchFlusher() {
        requestBufMu.Lock()
        if requestFlusher != nil {
                requestBufMu.Unlock()
                return
        }
...
// IMPL-007: Stop the request batch flush goroutine
func stopRequestBatchFlusher() {
        requestBufMu.Lock()
        defer requestBufMu.Unlock()
        if requestFlusher != nil {
                requestFlusher.Stop()
                requestFlusher = nil
                close(requestDone)
...
// IMPL-007: Flush buffered request trackers as a single batched message
func flushRequestBuf() {
        requestBufMu.Lock()
        if len(requestBuf) == 0 {
                requestBufMu.Unlock()
                return
        ...
        batch := requestBuf
        requestBuf = make([]statistic.Tracker, 0, len(batch))
        requestBufMu.Unlock()
...
// IMPL-007: Buffer a tracker instead of sending immediately
func bufferRequest(c statistic.Tracker) {
        requestBufMu.Lock()
...
func init() {
        ...
        // IMPL-007: start the request batch flusher on init
